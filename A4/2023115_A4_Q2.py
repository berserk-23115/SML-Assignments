...
def train_stump(X, y, rng=None, max_candidates=1000):
    ...
    for fi in range(X.shape[1]):
        ...
        # If too many candidates, randomly sample up to max_candidates
        if rng is not None and len(candidate_indices) > max_candidates:
            candidate_indices = rng.choice(candidate_indices, size=max_candidates, replace=False)
            candidate_indices = sorted(candidate_indices)  # Keep sorted order for efficiency
    ...
    return {'feature': bf, 'threshold': bt, 'left_value': blv, 
            'right_value': brv, 'ssr': bssr}
...
def gradient_boosting_stumps(X_train, y_train, X_val, y_val, X_test, y_test,
                             eta=0.01, n_stumps=300, use_candidate_sampling=False, rng=None):
    stumps = []
    ...
    for i in range(n_stumps):
        ...
        stump = train_stump(X_train, residuals_train, rng=rng if use_candidate_sampling else None)
        
        if stump['feature'] is None:
            print(f"Warning: Could not find valid split at iteration {i}")
            break
...
def predict_ensemble(stumps, X, eta):
    """Make predictions using ensemble of stumps."""
...
print("\n" + "="*70)
print("Testing with η = 0.01")
print("="*70)

np.random.seed(42)
rng = np.random.default_rng(42)

stumps, train_mse, val_mse, test_mse, best_iter, best_test_mse = gradient_boosting_stumps(
    Y_train, y_train, Y_val, y_val, Y_test, y_test,
    eta=0.01, n_stumps=300, use_candidate_sampling=False, rng=rng
)
...
# Plot validation MSE vs number of trees
...
for eta in learning_rates:
    ...
    stumps_eta, train_mse_eta, val_mse_eta, test_mse_eta, best_iter_eta, best_test_mse_eta = \
        gradient_boosting_stumps(Y_train, y_train, Y_val, y_val, Y_test, y_test,
                                 eta=eta, n_stumps=300, use_candidate_sampling=False, rng=rng)
    ...
    print(f"  Best val MSE: {results[eta]['best_val_mse']:.6f}")
...
for eta in learning_rates:
    r = results[eta]
    print(f"{eta:<15} {r['best_iter']+1:<12} {r['best_val_mse']:<15.6f} {r['best_test_mse']:<15.6f}")

plt.figure(figsize=(12, 7))
for eta in learning_rates:
    plt.plot(range(1, len(results[eta]['val_mse']) + 1), 
             results[eta]['val_mse'], 
...
print("\nPlot saved as 'validation_mse_all_etas.png'")
plt.show()
...
print("Plot saved as 'mse_comparison_all_etas.png'")
plt.show()